...
export default function OrderContent({ orderNumber }:{ orderNumber:string }){
    const [order, setOrder] = useState<any>(null)
    useEffect(()=>{ const raw = sessionStorage.getItem('lastOrder'); if(raw){ const o = JSON.parse(raw); if(o.orderNumber===orderNumber) setOrder(o) } },[orderNumber])

    if(!order) return <section className="mx-auto max-w-6xl px-4 py-10">Commande {orderNumber} confirmée.</section>
